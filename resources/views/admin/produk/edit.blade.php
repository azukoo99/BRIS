...
<section class="pt-32 pb-20 bg-surface min-h-screen">
    ...
    <div>
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">Edit Produk</h1>
        <p class="text-gray-500 text-sm">Ubah detail informasi produk.</p>
    </div>
</div>

<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <form action="{{ route('admin.produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="md:col-span-2">
                ...
                <input type="text" id="nama" name="nama" value="{{ old('nama', $produk->nama) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">
            ...
            <div>
                ...
                <input type="number" id="harga" name="harga" value="{{ old('harga', $produk->harga) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">
            ...
            <div>
                ...
                <input type="number" id="stok" name="stok" value="{{ old('stok', $produk->stok) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
                @error('deskripsi') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="gambar" class="block text-sm font-medium text-gray-700 mb-2">Gambar Produk (Kosongkan jika tidak diubah)</label>
                @if($produk->gambar)
                    <div class="mb-3">
                        <img src="{{ asset($produk->gambar) }}" alt="{{ $produk->nama }}" class="h-24 rounded-lg border border-gray-200">
                    </div>
                @endif
            ...
            <div>
                ...
                <select id="is_active" name="is_active" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all bg-white">
                    <option value="yes" {{ old('is_active', $produk->is_active) == 'yes' ? 'selected' : '' }}>Aktif (Tampilkan)</option>
                    <option value="no" {{ old('is_active', $produk->is_active) == 'no' ? 'selected' : '' }}>Tidak Aktif (Sembunyikan)</option>
                </select>
